...
function bots(etat){
  ...
  for (const l of etat.lignes){
    ...
    /* Le plancher part de l'or de DEPART, pas d'une fraction de celui-ci : dans
       les quinze premieres secondes, un tiers des gains ne fait que deux tours
       de guet, et un joueur qui envoie des la premiere seconde passe au
       travers. Un humain, lui, met presque toute sa mise de depart dans sa
       defense avant de penser a attaquer. */
    ...
    if (etat.pas >= l.prochainBot){
      ...
      /* On met de cote ce qui manque encore a la defense AVANT de calculer le
         budget d'envoi. Sans cette reserve, le bot depensait son or en
         monstres entre deux poses et n'avait que trois tours a la quinzieme
         seconde — un joueur qui envoie des la premiere seconde le pliait avant
         qu'il n'ait rien bati. Un humain construit d'abord, attaque ensuite. */
      const dette = Math.max(0, plancherTours - l.depenseTours);
      let budget = Math.floor(Math.max(0, l.or - dette) * d.partEnvois);
      const cible = etat.lignes[voisin(etat, l.i)];
